...
async function copyTextToClipboard(text) {
  ...
  try {
    ...
    toast("已复制到剪贴板。", "success", 1800);
    return true;
  } catch (error) {
    toast("复制失败，请手动复制。", "error", 2600);
    return false;
  }
}
...
function renderCopyButton(message) {
  return `<button class="copy-btn" type="button" data-copy-message-id="${message.id}" title="复制文本">复制</button>`;
}

function getMessageCopyText(message) {
  if (!message) return "";
  if (message.role === "user" || message.role === "error") return String(message.content || "").trim();
...
function renderAssistantMessage(message) {
  ...
  return `
    <article class="message assistant" data-message-id="${message.id}">
      <div class="avatar">AI</div>
      <div class="message-content">
        ${message.content ? `<div class="message-main-text">${escapeHtml(message.content)}</div>` : ""}
        ${imageGrid}
        <div class="message-footer">
          ${meta}
          <div class="message-tools">${renderCopyButton(message)}</div>
...
function bindEvents() {
  ...
  els.messageList.addEventListener("click", async (event) => {
    const copyTarget = event.target.closest("[data-copy-message-id]");
    if (!copyTarget) return;
    const conversation = getCurrentConversation();
    const message = conversation?.messages?.find((item) => item.id === copyTarget.dataset.copyMessageId);
