...
async function exportAllPresets() {
  ...
  if (!window.JSZip) {
    toast("未加载 ZIP 组件，无法导出全部预设。", "error");
    return;
  }
  ...
  zip.file("all-presets.json", JSON.stringify({
    schema: PRESET_BUNDLE_SCHEMA,
    version: 1,
    exportedAt,
    presets,
  ...
  presets.forEach((preset, index) => {
    zip.file(`presets/${String(index + 1).padStart(2, "0")}-${sanitizeFilename(preset.name)}.json`, JSON.stringify(buildPresetExportObject(preset), null, 2));
  });
...
async function readPresetFiles(files) {
  ...
  for (const file of files) {
    ...
    if (lower.endsWith(".zip")) {
      if (!window.JSZip) throw new Error("未加载 ZIP 组件，无法导入 ZIP。 ");
